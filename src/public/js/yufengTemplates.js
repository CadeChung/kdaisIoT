$(document).ready(() => {
    ...
    let yufengECGauge = createJustGage('yufengECGauge', '2', 'mS/cm');
    let yufengTempGauge = createJustGage('yufengTempGauge', '40', '°C');
    let yufengHumdGauge = createJustGage('yufengHumdGauge', '100', '%');
    ...
    const updateTime = () => {
        ...
        $('#yufeng-currentTime').text(dateString);
    }
    ...
    const getYufengWeatherData = () => {
        $.getJSON('http://127.0.0.1:5000/api/last_data', (data) => {
            ...
            yufengECGauge.refresh(data.ecData.EC_Gauge);
            yufengTempGauge.refresh(data.weatherData.TEMP_Gauge);
            yufengHumdGauge.refresh(data.weatherData.HUMD_Gauge);
